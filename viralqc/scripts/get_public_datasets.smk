...
# Build input list dynamically based on available datasets
def get_all_inputs():
    inputs = {
        # Nextclade datasets (always required)
    ...
    # GitHub datasets and minimizer index (only when github datasets exist)
    if parameters.has_github_datasets:
        inputs.update({
...
# Only define get_github_databases rule if there are github datasets
if parameters.has_github_datasets:
